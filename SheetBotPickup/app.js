...
function saveConfig() {
  localStorage.setItem('sheet_bot_pickup_config', JSON.stringify(botState.config));
}

function loadConfig() {
  const saved = localStorage.getItem('sheet_bot_pickup_config');
  if (saved) {
    try {
...
function updateGasCode() {
  ...
  const code = `// ========================================================
// SHEETBOT - GOOGLE APPS SCRIPT BACKEND TELEGRAM BOT (PICKUP)
// ========================================================
  ...
  try {
    ...
    if (text.startsWith("/start")) {
      var welcome = "👋 <b>Halo " + username + "!</b>\\n\\n" +
                    "Saya adalah bot integrasi Google Sheets untuk <b>Pickup Resi</b>.\\n" +
                    "Kirimkan data hasil <b>scan barcode resi</b> atau ketik detail pickup ke bot ini. Data Anda akan langsung masuk ke Google Sheets.\\n\\n" +
                    "📌 <b>Format Input Pickup Multi-Resi:</b>\\n" +
                    "<code>Pickup msotomotif\\n" +
                    "29 Mei 2026\\n" +
                    "Trip 1\\n" +
                    "1. SPXID063846639495\\n" +
                    "2. SPXID062327764495\\n" +
                    "Keterangan: Paket sudah di-pickup.</code>\\n\\n" +
    ...
    var reply = "";
    if (savedCount > 0) {
      reply += "✅ <b>" + savedCount + " Data Resi Pickup Berhasil Disimpan!</b>\\n\\n" +
...
function parseResi(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // 1. Ekstrak Nama Seller dari "Pickup [Nama Seller]" atau "Pick up [Nama Seller]"
    if (lowerLine.indexOf("pickup ") === 0) {
      seller = line.substring(7).trim();
    } else if (lowerLine.indexOf("pick up ") === 0) {
      seller = line.substring(8).trim();
    }
    // 2. Ekstrak Trip
...
async function setWebhook() {
  ...
  try {
    ...
    if (data.ok) {
      showStatus('🎉 Webhook Sukses Terdaftar! Bot Telegram Pickup Anda sekarang terhubung ke Google Sheets.', 'success');
...
async function deleteWebhook() {
  ...
  try {
    ...
    if (data.ok) {
      showStatus('🧹 Webhook berhasil dinonaktifkan. Bot Telegram Pickup tidak lagi terhubung ke Google Sheets.', 'success');
